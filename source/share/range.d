...
template skip(Range)
    if(isInputRange!Range)
{
    auto skip(Range range, size_t amount)
    {
        return Skip(range, amount);
    }
    ...
    struct Skip
    {
        this(Range range, size_t amount)
        {
            _range = range;
            _amount = amount;
            popNotNeededElements;
        }

        private Range _range;
        private size_t _amount;
        ...
        private void popNotNeededElements()
        {
            if(!_skipped)
            {
                foreach(i; 0 .. _amount)
...
@("Skip should skip the first elements")
unittest
{
    import fluent.asserts;
    [1, 2, 3, 4].skip(2).should.equal([3, 4]);
}

@("Skip does not throw when the skip is larger than the range")
unittest
{
    import fluent.asserts;
    [1, 2, 3, 4].skip(5).empty.should.equal(true);
